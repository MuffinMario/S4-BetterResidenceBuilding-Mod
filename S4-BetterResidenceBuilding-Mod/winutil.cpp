...
namespace Winutil {
	...
	*/
	BOOL processIs64Bit(DWORD pid)
	{
		...
		if (IsWow64Process(process, &isWOW64) == FALSE) {
			return FALSE; // System does not contain this function, I can assume this only happens on very old (not all) 32-bit windows systems
		}

		if (isWOW64) { // process is running under WOW64
			return FALSE; // process is not 64-bit running on 64-bit
		}
		else { // process runs in native environment
	...
	*/
	int messageBoxA(const STRING8& message, const STRING8& title, HWND window, MessageBoxTypes type, MessageBoxIcons mbi, MessageBoxDefaultButtons defaultbuttons, MessageBoxModalty modalty, MessageBoxOtherSingleOrList others)
	{
		...
		catch (const std::bad_variant_access& bva) { // bad overhead tbh
	...
	int messageBoxW(const WSTRING& message, const WSTRING& title, HWND window, MessageBoxTypes type, MessageBoxIcons mbi, MessageBoxDefaultButtons defaultbuttons, MessageBoxModalty modalty, MessageBoxOtherSingleOrList others)
	{
		UINT typeParam = static_cast<UINT>(type) | static_cast<UINT>(mbi) | static_cast<UINT>(defaultbuttons)
			| static_cast<UINT>(modalty);
		try {
	...
	bool processContainsModuleA(DWORD pid, STRING8 moduleName)
	{
		...
		if (!process)
			throw CProcessNotFoundException("Could not OpenProcess with information PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE");
		HMODULE aMods[1024];
		DWORD cbNeeded = 0;
